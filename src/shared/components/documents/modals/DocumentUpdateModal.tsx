...
export default function DocumentUpdateModal({
  ...
  const [tempStates, setTempStates] = useState<TempModalStates>(() => {
    const destinatarios =
      documento?.tipoDocumento === 'Ofício Circular' ? documento?.destinatario : undefined;
    ...
    switch (modalType) {
      case 'finalizacao':
        if (tempStates.dataFinalizacaoFormatted) {
          const validation = validateDateNotFuture(tempStates.dataFinalizacaoFormatted);
          if (!validation.isValid) {
            errors.push(validation.errorMessage || 'Data de finalização inválida');
          }
        }
        break;
      ...
      case 'oficio':
        if (tempStates.dataEnvioFormatted) {
          const validation = validateDateNotFuture(tempStates.dataEnvioFormatted);
          if (!validation.isValid) {
            errors.push('Data de envio não pode ser posterior à data atual.');
          }
        }
        if (tempStates.dataRespostaFormatted) {
          const validation = validateDateNotFuture(tempStates.dataRespostaFormatted);
          if (!validation.isValid) {
            errors.push('Data de resposta não pode ser posterior à data atual.');
          }
        }
        break;
      ...
      case 'oficio_circular_outros':
        tempStates.destinatariosData.forEach((dest, index) => {
          ...
          if (dest.dataRespostaFormatted) {
            const validation = validateDateNotFuture(dest.dataRespostaFormatted);
            if (!validation.isValid) {
              errors.push(
      ...
      default:
        return (
          <div className={styles.formGroup}>
            <p className={styles.noData}>Necessária definição de modal para esse documento.</p>
          </div>
        );
    }
  };
