...
export default function OficioAutosCircunstanciadosModal({
  ...
  return (
    <>
      ...
      <div className={styles.formGroup}>
        <label className={styles.formLabel}>Selecione os Autos Circunstanciados</label>
          ...
          {autosDemanda.length > 0 ? (
            autosDemanda.map(doc => (
              <label key={doc.id} className={styles.checkboxLabel}>
                <input
                  type='checkbox'
                  checked={tempStates.selectedAutosCircunstanciados.includes(doc.id.toString())}
                  onChange={e => handleCheckboxChange(doc.id.toString(), e.target.checked)}
            ...
            <p className={styles.noData}>Nenhum auto circunstanciado encontrado nesta demanda.</p>
          )}
        </div>
      </div>
    </>
