...
export default function OficioOutrosModal({ tempStates, setTempStates }: ModalContentProps) {
  const handleDateChange = (value: string, isFormatted = true) => {
    if (isFormatted) {
      ...
      setTempStates(prev => ({
        ...
        dataEnvio:
          formattedValue.length === 10 ? convertToHTMLDate(formattedValue) : formattedValue,
      }));
    } else {
      // Input HTML5 date
      setTempStates(prev => ({
