...
export default function OficioRelatorioInteligenciaModal({
  ...
  return (
    <>
      ...
      <div className={styles.formGroup}>
        <label className={styles.formLabel}>Selecione os Relatórios de Inteligência</label>
          ...
          {relatoriosDemanda.length > 0 ? (
            relatoriosDemanda.map(doc => (
              <label key={doc.id} className={styles.checkboxLabel}>
                <input
                  type='checkbox'
                  checked={tempStates.selectedRelatoriosInteligencia.includes(doc.id.toString())}
