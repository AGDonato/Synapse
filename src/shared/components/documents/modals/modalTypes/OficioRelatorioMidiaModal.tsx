...
export default function OficioRelatorioMidiaModal({
  ...
  const handleDataEnvioChange = (value: string) => {
    ...
    setTempStates(prev => ({
      ...
      dataEnvio: formatted.length === 10 ? convertToHTMLDate(formatted) : formatted,
    }));
  };
  ...
  return (
    <>
      ...
      <div className={styles.formGroup}>
        <label className={styles.formLabel}>Selecione os Relatórios Técnicos</label>
          ...
          {relatoriosTecnicos.length > 0 ? (
            relatoriosTecnicos.map(doc => (
              <label key={doc.id} className={styles.checkboxLabel}>
                <input
                  type='checkbox'
                  checked={tempStates.selectedRelatoriosTecnicos.includes(doc.id.toString())}
                  onChange={e => handleRelatorioChange(doc.id.toString(), e.target.checked)}
          ...
          {midias.length > 0 ? (
            midias.map(doc => (
              <label key={doc.id} className={styles.checkboxLabel}>
                <input
                  type='checkbox'
                  checked={tempStates.selectedMidias.includes(doc.id.toString())}
                  onChange={e => handleMidiaChange(doc.id.toString(), e.target.checked)}
                  className={styles.checkbox}
                />
                <span className={styles.checkboxText}>
                  {doc.numeroDocumento}
