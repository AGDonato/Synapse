...
export default function OficioCircularModal({ tempStates, setTempStates }: ModalContentProps) {
  const handleDestinatarioDataEnvioChange = (index: number, value: string) => {
    ...
    newData[index].dataEnvio = formatted.length === 10 ? convertToHTMLDate(formatted) : formatted;
    setTempStates(prev => ({ ...prev, destinatariosData: newData }));
  };

  const handleDestinatarioDataEnvioCalendarChange = (index: number, value: string) => {
    const newData = [...tempStates.destinatariosData];
  ...
  const handleDestinatarioDataRespostaChange = (index: number, value: string) => {
    let formatted = value.replace(/\D/g, '');
    if (formatted.length >= 3 && formatted.length <= 4) {
      formatted = `${formatted.slice(0, 2)}/${formatted.slice(2)}`;
    } else if (formatted.length >= 5) {
  ...
  const handleDestinatarioDataRespostaCalendarChange = (index: number, value: string) => {
    const newData = [...tempStates.destinatariosData];
            ...
            <div className={styles.formGroup}>
              ...
              <div className={styles.dateInputWrapper}>
                <input
                  ...
                  onChange={e => handleDestinatarioDataEnvioChange(index, e.target.value)}
                  className={styles.formInput}
                ...
                <input
                  ...
                  onChange={e => handleDestinatarioDataEnvioCalendarChange(index, e.target.value)}
            ...
            <div className={styles.formGroup}>
              ...
              <div className={styles.dateInputWrapper}>
                <input
                  ...
                  onChange={e => handleDestinatarioDataRespostaChange(index, e.target.value)}
                  className={styles.formInput}
                  placeholder='dd/mm/aaaa'
                  maxLength={10}
                />
                <input
                  ...
                  onChange={e =>
                    handleDestinatarioDataRespostaCalendarChange(index, e.target.value)
                  }
                  className={styles.hiddenDateInput}
                  tabIndex={-1}
                />
