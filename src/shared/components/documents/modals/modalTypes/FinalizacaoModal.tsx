...
export default function FinalizacaoModal({ tempStates, setTempStates }: ModalContentProps) {
  const handleDataFinalizacaoChange = (value: string) => {
    ...
    setTempStates(prev => ({
      ...
      dataFinalizacao: formatted.length === 10 ? convertToHTMLDate(formatted) : formatted,
    }));
  };
