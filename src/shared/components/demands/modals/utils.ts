...
export const getModalType = (demanda: Demanda | null, context?: string): DemandModalType => {
  if (!demanda) {
    return 'default';
  }
  ...
  if (context) {
    if (context === 'reopen') {
      return 'reopen_demand';
    }
    if (context === 'final_date') {
      return 'final_date';
    }
    if (context === 'status') {
...
export const initializeTempStates = (demanda: Demanda | null): TempDemandStates => {
  ...
  return {
    ...
    dataReaberturaFormatted: formatDateForDisplay(demanda?.dataReabertura || ''),
  ...
  switch (modalType) {
    case 'final_date':
      ...
      return (
        ...
        tempStates.dataReaberturaFormatted !== initialStates.dataReaberturaFormatted ||
        tempStates.novaDataFinalFormatted !== initialStates.novaDataFinalFormatted
      );
    ...
    case 'reopen_demand':
      ...
      if (initialStates.dataReabertura) {
        return tempStates.novaDataFinalFormatted !== initialStates.novaDataFinalFormatted;
      }
      return (
        tempStates.isReaberto !== initialStates.isReaberto ||
        tempStates.dataReaberturaFormatted !== initialStates.dataReaberturaFormatted ||
    ...
    switch (dateType) {
      ...
      case 'reabertura':
        ...
        if (demanda.dataFinal) {
          ...
          const finalDateStr = tempStates?.dataFinalFormatted || demanda.dataFinal;
      ...
      case 'nova_final': {
        ...
        const reaberturaStr = tempStates?.dataReabertura || demanda.dataReabertura || '';
        const reaberturaTemp = parseDate(reaberturaStr);
        if (reaberturaTemp && targetDate && targetDate < reaberturaTemp) {
          return {
            isValid: false,
            error: 'A nova data final não pode ser anterior à data de reabertura.',
          };
        }
        break;
      }
    ...
    case 'final_date':
      ...
      if (tempStates.isReaberto) {
        ...
        if (!tempStates.dataReabertura) {
          return {
            data: null,
            error: 'Data de reabertura é obrigatória quando marcado como reaberto.',
          };
        }
        ...
        updateData.dataReabertura =
          convertToBrazilianDate(tempStates.dataReabertura) || tempStates.dataReabertura;
        updateData.novaDataFinal = tempStates.novaDataFinal
          ? convertToBrazilianDate(tempStates.novaDataFinal) || tempStates.novaDataFinal
          : '';
      ...
      // Se não está reaberto, processa data final normalmente
        ...
        // Validar data final
        const validation = validateDate(tempStates.dataFinal, demanda, 'final', tempStates);
        ...
        updateData.dataFinal = convertToBrazilianDate(tempStates.dataFinal) || tempStates.dataFinal;
        updateData.status = 'Finalizada' as const;
      } else {
