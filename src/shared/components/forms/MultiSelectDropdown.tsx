...
export default function MultiSelectDropdown({
  ...
  const selectedFilteredOptions = filteredOptions.filter(option => isOptionSelected(option));

  const availableFilteredOptions = filteredOptions.filter(option => !isOptionSelected(option));

  // Combinar as listas para navegação (selecionados primeiro)
  const allFilteredOptions = [...selectedFilteredOptions, ...availableFilteredOptions];
  ...
  }, [selectedIndex, isOpen, selectedFilteredOptions.length, availableFilteredOptions.length]);
        ...
        onKeyDown={e => {
          ...
          switch (e.key) {
            case 'ArrowDown': {
              e.preventDefault();
              const nextIndex =
                selectedIndex < allFilteredOptions.length - 1 ? selectedIndex + 1 : selectedIndex;
              setSelectedIndex(nextIndex);
              break;
            }
            case 'ArrowUp': {
            ...
            case ' ': {
              e.preventDefault();
              if (selectedIndex >= 0 && selectedIndex < allFilteredOptions.length) {
