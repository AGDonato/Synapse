 ...
 * - Suporte a múltiplos provedores SSO (LDAP, OAuth2, SAML, Node.js)
 ...
 * - Node.js JWT: Integração com backend Node.js/Express nativo
...
class SSOAdapter {
   ...
   * baseado na variável AUTH_TYPE ou usa Node.js JWT como fallback.
   *
   * @private
   */
   ...
   *
   * @param type - Tipo do provedor ('nodejs', 'ldap', 'oauth2', 'saml')
  ...
  private async initializeOAuth2(): Promise<SSOAuthResponse> {
    ...
    if (token) {
      try {
        const response = await nodejsApiClient.get('/auth/oauth2/me', {
  ...
  private async loginOAuth2(credentials: SSOCredentials): Promise<SSOAuthResponse> {
    if (credentials.code) {
      ...
      try {
        const response = await nodejsApiClient.post('/auth/oauth2/callback', {
          json: {
  ...
  private async initializeSAML(): Promise<SSOAuthResponse> {
    ...
    try {
      const response = await nodejsApiClient.get('/auth/saml/check');
      const data = (await response.json()) as any;
  ...
  private async loginSAML(credentials: SSOCredentials): Promise<SSOAuthResponse> {
    if (credentials.assertion) {
      ...
      try {
        const response = await nodejsApiClient.post('/auth/saml/acs', {
          json: {
  ...
  private async logoutSAML(): Promise<SSOAuthResponse> {
    try {
      await nodejsApiClient.post('/auth/saml/logout');
    } catch (error) {
   ...
   * Suportado por Node.js JWT e OAuth2 (refresh tokens).
   *
