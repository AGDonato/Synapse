 ...
 * - Suporte completo a Node.js/Express, LDAP, OAuth2 e SAML
 ...
 * - Node.js/Express Backend: Integração direta com APIs Node.js/Express
...
export const AUTH_ENV_KEYS = {
  ...
  /** Configurações para backend Node.js/Express */
  NODEJS_BASE_URL: 'VITE_NODEJS_BASE_URL',
  NODEJS_LOGIN_ENDPOINT: 'VITE_NODEJS_LOGIN_ENDPOINT',
  NODEJS_REFRESH_ENDPOINT: 'VITE_NODEJS_REFRESH_ENDPOINT',
  NODEJS_LOGOUT_ENDPOINT: 'VITE_NODEJS_LOGOUT_ENDPOINT',
  NODEJS_PROFILE_ENDPOINT: 'VITE_NODEJS_PROFILE_ENDPOINT',
  NODEJS_VERIFY_ENDPOINT: 'VITE_NODEJS_VERIFY_ENDPOINT',
  NODEJS_TIMEOUT: 'VITE_NODEJS_TIMEOUT',
...
/**
 * Cria configuração otimizada para integração com Node.js/Express Backend
 *
 * @param baseUrl - URL base opcional (sobrescreve variável de ambiente)
 * @returns Configuração completa para autenticação Node.js
 *
 * @example
 * ```typescript
 * // Usando variáveis de ambiente
 * const config = createNodeJSConfig();
 *
 * // Sobrescrevendo URL
 * const config = createNodeJSConfig('https://api.meudominio.com');
 * ```
 */
export function createNodeJSConfig(baseUrl?: string): AuthProviderConfig {
  const url = baseUrl || import.meta.env[AUTH_ENV_KEYS.NODEJS_BASE_URL] || 'http://localhost:3001';

  return {
    provider: 'custom_nodejs',
    config: {
      baseUrl: url,
      endpoints: {
        login: import.meta.env[AUTH_ENV_KEYS.NODEJS_LOGIN_ENDPOINT] || '/api/auth/login',
        refresh: import.meta.env[AUTH_ENV_KEYS.NODEJS_REFRESH_ENDPOINT] || '/api/auth/refresh',
        logout: import.meta.env[AUTH_ENV_KEYS.NODEJS_LOGOUT_ENDPOINT] || '/api/auth/logout',
        profile: import.meta.env[AUTH_ENV_KEYS.NODEJS_PROFILE_ENDPOINT] || '/api/auth/me',
        verify: import.meta.env[AUTH_ENV_KEYS.NODEJS_VERIFY_ENDPOINT] || '/api/auth/verify',
      },
      headers: {
      ...
      timeout: Number(import.meta.env[AUTH_ENV_KEYS.NODEJS_TIMEOUT]) || 15000,
    } satisfies CustomNodeJSConfig,
 ...
 * // VITE_NODEJS_BASE_URL=https://api.empresa.com
 *
...
export function validateAuthConfig(config: AuthProviderConfig): {
  ...
  switch (config.provider) {
    case 'custom_nodejs': {
      const nodejsConfig = config.config as CustomNodeJSConfig;

      if (!nodejsConfig.baseUrl) {
        errors.push('URL base Node.js é obrigatória');
      } else if (!nodejsConfig.baseUrl.startsWith('http')) {
        errors.push('URL base Node.js deve começar com http:// ou https://');
      }

      if (nodejsConfig.baseUrl?.startsWith('http://') && import.meta.env.PROD) {
        warnings.push('Usar HTTP em produção não é recomendado');
      }

      if (nodejsConfig.timeout && nodejsConfig.timeout < 5000) {
 ...
 * const nodejsConfig = authConfig.createNodeJSConfig('https://api.com');
...
export default {
  ...
  validateAuthConfig,
  /** Factory específico para Node.js/Express */
  createNodeJSConfig,
