 ...
 * - frame-ancestors: Configure no servidor (Apache/Nginx/Node.js)
 ...
 * - Node.js/Express: app.use middleware para headers
...
class ContentSecurityPolicy {
  ...
  getServerConfigInstructions(): {
    ...
    return {
      ...
      nodejs: `app.use((req, res, next) => {
  res.setHeader("Content-Security-Policy", "${fullCSP}");
  next();
});`,
    };
  }
  ...
  applyMetaTag(): void {
    ...
    if (import.meta.env.DEV) {
      ...
      logger.info('Node.js/Express:', config.nodejs);
