...
export const QuickManagementSection: React.FC<QuickManagementSectionProps> = memo(({
  ...
  const demandasFiltradas = useMemo(() => {
    ...
    if (debouncedDocumentos) {
      ...
      resultado = resultado.filter((d: Demanda) => {
        ...
        return documentosDaDemanda.some(documento => {
          return documento.codigoRastreio?.toLowerCase().includes(termoBuscaDocumentos) ||
    ...
    return resultado;
  }, [demandas, filtros.analista, filtros.dataInicio, filtros.dataFim, debouncedReferencia, debouncedDocumentos, getDocumentosByDemandaId, isDateInRange]);

  // Dados filtrados para documentos
  const documentosFiltrados = useMemo(() => {
    ...
    // Aplicar outros filtros similares...
  ...
  // Função para obter abreviação do tipo de documento - sem useCallback (função pura simples)
  ...
  // Renderizar status do documento - sem useCallback (não altera dependências)
  ...
  const handleSgedClick = useCallback((e: React.MouseEvent, demanda: Demanda) => {
    e.stopPropagation();
    navigate(`/demandas/${demanda.id}`);
  }, [navigate]);

  const handleDocumentNumberClick = useCallback((e: React.MouseEvent, documento: DocumentoDemanda) => {
    e.stopPropagation();
