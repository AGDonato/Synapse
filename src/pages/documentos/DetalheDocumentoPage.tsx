...
import { getVisibleFields } from '../../shared/components/documents/modals/utils';
...
import type { DestinatarioDocumento, DocumentoDemanda } from '../../shared/data/mockDocumentos';
import { formatDateToDDMMYYYYOrPlaceholder } from '../../shared/utils/dateUtils';
import {
  type DocumentStatus,
  getDocumentStatus,
  getIndividualRecipientStatus,
  getStatusColor,
} from '../../shared/utils/documentStatusUtils';
import styles from './DetalheDocumentoPage.module.css';

type SortConfig = {
  key: 'numeroDocumento' | 'tipoDocumento' | 'assunto' | 'destinatario' | 'status';
  direction: 'asc' | 'desc';
} | null;
...
export default function DetalheDocumentoPage() {
  ...
  const [destinatarioStatusAtivo, setDestinatarioStatusAtivo] = useState<number>(0);
  ...
  const hasPesquisaData = () => {
    ...
    return documentoBase.pesquisas.some(pesquisa => pesquisa.tipo || pesquisa.identificador);
  };
  ...
  const renderInformacoesAdicionais = () => {
      ...
      return (
        <dl className={styles.infoList}>
          <div className={styles.infoItem}>
            ...
            <dd className={styles.infoValue}>{documentoBase.apresentouDefeito ? 'Sim' : 'Não'}</dd>
          </div>
        </dl>
      );
    }
    ...
    if (
      ...
      ((documentoBase.selectedRelatoriosTecnicos &&
    ...
    if (
      ...
      documentoBase.selectedRelatoriosInteligencia &&
    ...
    if (
      ...
      documentoBase.selectedAutosCircunstanciados &&
    ...
    if (
      ...
      documentoBase.selectedDecisoes &&
  ...
  const renderOficioCircularStatusCompleto = () => {
    ...
    const destinatarioAtual = destinatariosData[destinatarioStatusAtivo] || destinatariosData[0];
        ...
        <div className={styles.destinatarioConteudo}>
          <dl className={styles.infoList}>
            ...
            <div className={styles.infoItem}>
              ...
              <dd className={styles.infoValue}>
                {formatDateToDDMMYYYYOrPlaceholder(destinatarioAtual.dataResposta)}
              </dd>
            </div>
            <div className={styles.infoItem}>
            ...
            <div className={styles.infoItem}>
              ...
              <dd className={styles.infoValue}>
                {(() => {
                  const status = getIndividualRecipientStatus(destinatarioAtual);
                  const statusClass =
                    status === 'Respondido'
          ...
          <div className={styles.infoItem}>
            ...
            <dd className={styles.infoValue}>
              {documentoBase.respondido ? (
                <span className={`${styles.statusBadge} ${styles.statusSuccess}`}>Respondido</span>
              ) : (
  ...
  const renderOficioMidiaContent = () => {
    ...
    const midiasSelecionadas = midiasIds
      .map(id =>
        documentosDemanda.find(doc => doc.id.toString() === id && doc.tipoDocumento === 'Mídia')
      )
            ...
            {midiasSelecionadas.map(midia => (
              <div key={midia.id} className={styles.midiaItem}>
                ...
                <div className={styles.midiaDetalhes}>
                  Hash:{' '}
                  <span className={styles.hashText}>{midia.hashMidia || 'Não informado'}</span>
                  <br />
                  Senha: {midia.senhaMidia || 'Não informado'}
                  <br />
                  Status: {midia.apresentouDefeito ? 'Apresentou defeito' : 'Sem defeito'}
                </div>
              </div>
            ))}
          </dd>
    ...
    const relatoriosSelecionados = relatoriosIds
      .map(id =>
        documentosDemanda.find(
          doc => doc.id.toString() === id && doc.tipoDocumento === 'Relatório Técnico'
        )
      )
      .filter((doc): doc is NonNullable<typeof doc> => doc !== undefined);
  ...
  const renderOficioRelatorioInteligenciaContent = () => {
    ...
    const relatoriosSelecionados = relatoriosIds
      .map(id =>
        documentosDemanda.find(
          doc => doc.id.toString() === id && doc.tipoDocumento === 'Relatório de Inteligência'
        )
      )
      .filter((doc): doc is NonNullable<typeof doc> => doc !== undefined);
    ...
    return (
      <dl className={styles.infoList}>
        ...
        <div className={styles.infoItem}>
          <dt className={styles.infoLabel}>Relatórios de inteligência selecionados</dt>
          <dd className={styles.infoValue}>
            {relatoriosSelecionados.map(relatorio => (
              <div key={relatorio.id} className={styles.relatorioItem}>
  ...
  const renderOficioAutosContent = () => {
    ...
    const autosSelecionados = autosIds
      .map(id =>
        documentosDemanda.find(
          doc => doc.id.toString() === id && doc.tipoDocumento === 'Autos Circunstanciados'
        )
      )
      .filter((doc): doc is NonNullable<typeof doc> => doc !== undefined);
    ...
    return (
      <dl className={styles.infoList}>
        ...
        <div className={styles.infoItem}>
          <dt className={styles.infoLabel}>Autos circunstanciados selecionados</dt>
          <dd className={styles.infoValue}>
    ...
    if (decisoesSelecionadas.length === 0) {
      return (
        <dl className={styles.infoList}>
          ...
          <div className={styles.infoItem}>
            <dt className={styles.infoLabel}>Destinatários que não cumpriram a decisão judicial</dt>
  ...
  const renderOficioSimples = (doc: DocumentoDemanda) => {
    const formatCodigoRastreio = () => {
      if (doc.naopossuiRastreio) {
        return 'Não possui rastreio';
      }
  ...
  const renderOficioCircularComplexo = (doc: DocumentoDemanda) => {
    ...
    const destinatariosPendentesEnviados =
      doc.destinatariosData?.filter(
        dest => (!dest.respondido || !dest.dataResposta) && dest.dataEnvio && dest.dataEnvio !== ''
      ) || [];
  ...
  const documentosDemanda = useMemo(() => {
    ...
    const filtered = todosDocumentos.filter(doc => doc.demandaId === documentoBase.demandaId);
      ...
      return (
        <dl className={styles.infoList}>
          ...
          <div className={styles.infoItem}>
            ...
            <dd className={styles.infoValue}>{documentoBase?.autoridade || 'Não informado'}</dd>
          </div>
          <div className={styles.infoItem}>
            <dt className={styles.infoLabel}>Órgão Judicial</dt>
            <dd className={styles.infoValue}>{documentoBase?.orgaoJudicial || 'Não informado'}</dd>
          </div>
        </dl>
      );
    }
      ...
      <div className={styles.oficioCircularCarrossel}>
        ...
        <div className={styles.destinatarioConteudo}>
          <dl className={styles.infoList}>
            ...
            <div className={styles.infoItem}>
              <dt className={styles.infoLabel}>Autoridade</dt>
              <dd className={styles.infoValue}>{versaoAtual.autoridade || 'Não informado'}</dd>
            </div>
            <div className={styles.infoItem}>
              <dt className={styles.infoLabel}>Órgão Judicial</dt>
              <dd className={styles.infoValue}>{versaoAtual.orgaoJudicial || 'Não informado'}</dd>
            </div>
          </dl>
        </div>
          ...
          <div key={index} className={styles.pesquisaItem}>
            ...
            <dl className={styles.infoList}>
              <div className={styles.infoItem}>
                ...
                <dd className={styles.infoValue}>{pesquisa.identificador || 'Não informado'}</dd>
              </div>
  ...
  const calculateMidiaHeights = useCallback(() => {
    if (!isMidiaDocument() || !cardInformacoesRef.current) {
      return;
    }
    ...
    if (alturaDireita < alturaEsquerda) {
      ...
      if (totalAlturaOriginal > 0) {
        ...
        const proporcaoComplementares = alturaInfoComplementares / totalAlturaOriginal;
  ...
  const calculateOficioHeights = useCallback(() => {
    ...
    if (numCards === 2) {
      ...
      if (alturaPesquisa > alturaEsquerda) {
        ...
        const alturaDisponivel = alturaEsquerda - gap;
        const totalAlturaOriginal = alturaInformacoes + alturaInfoComplementares;
      ...
      const maiorAlturaInferior = Math.max(alturaInfoComplementares, alturaDecisaoJudicial);
        ...
        <dl className={styles.infoList}>
          {demanda && (
            <div className={styles.infoItem}>
              ...
              <dd className={styles.infoValue}>
                <Link to={`/demandas/${demanda.id}`} className={styles.linkDemanda}>
                  {demanda.sged}
                </Link>
              </dd>
            </div>
          ...
          {documentoBase.tipoDocumento !== 'Mídia' && (
            <div className={styles.infoItem}>
              ...
              <dd className={styles.infoValue}>
                ...
                {documentoBase.assunto === 'Outros' && documentoBase.assuntoOutros && (
                  <span className={styles.assuntoOutros}> - {documentoBase.assuntoOutros}</span>
                )}
              </dd>
            </div>
          )}
          <div className={styles.infoItem}>
            ...
            <dd className={styles.infoValue}>{formatDestinatarios(documentoBase.destinatario)}</dd>
          </div>
          <div className={styles.infoItem}>
            <dt className={styles.infoLabel}>Endereçamento</dt>
            <dd className={styles.infoValue}>{documentoBase.enderecamento || 'Não informado'}</dd>
          </div>
          <div className={styles.infoItem}>
            <dt className={styles.infoLabel}>Analista</dt>
            <dd className={styles.infoValue}>{documentoBase.analista || 'Não informado'}</dd>
          </div>
          <div className={styles.infoItem}>
            <dt className={styles.infoLabel}>Ano do Documento</dt>
            <dd className={styles.infoValue}>{documentoBase.anoDocumento || 'Não informado'}</dd>
          </div>
        </dl>
      ),
    };
        ...
        content: (
          <dl className={styles.infoList}>
            <div className={styles.infoItem}>
              ...
              <dd className={styles.infoValue}>{documentoBase.tipoMidia || 'Não informado'}</dd>
            </div>
            <div className={styles.infoItem}>
              <dt className={styles.infoLabel}>Tamanho</dt>
              <dd className={styles.infoValue}>{documentoBase.tamanhoMidia || 'Não informado'}</dd>
            </div>
            <div className={styles.infoItem}>
            ...
            <div className={styles.infoItem}>
              ...
              <dd className={styles.infoValue}>{documentoBase.senhaMidia || 'Não informado'}</dd>
            </div>
          </dl>
        ),
      };
          ...
          <h1>
            <div className={styles.titleWithStatus}>
              <div
                ...
                style={{
                  backgroundColor: getStatusColor(getDocumentStatus(documentoBase)),
                }}
              ...
              return (
                ...
                >
                  <div className={styles.cardHeader}>
                    <div className={styles.cardIcon}>{renderIcon(infoCard.icon)}</div>
          ...
          <div className={styles.midiaColumnRight}>
            {(() => {
              ...
              const rightCards = cards.filter(
                card => card.id === 'dados_midia' || card.id === 'informacoes_adicionais'
              );
        ...
        (() => {
          ...
          if (numCards === 2) {
            ...
            const leftCards = cards.filter(
              card => card.id === 'informacoes' || card.id === 'informacoes_adicionais'
            );
            const rightCards = cards.filter(card => card.id === 'dados_pesquisa');
                ...
                <div className={styles.oficioColumnLeft3Cards}>
                  {leftCards.map(card => (
                    <div
                      ...
                      style={{
                        height:
                          card.id === 'informacoes' && calculatedOficioHeights.informacoes
            ...
            const rightCards = cards.filter(
              card => card.id === 'dados_pesquisa' || card.id === 'dados_decisao_judicial'
            );
                ...
                <div className={styles.oficioColumnRight4Cards}>
                  {rightCards.map(card => (
                    <div
                      ...
                      ref={card.ref}
                      className={`${styles.infoCard} ${styles[card.color]} ${card.className || ''} ${
                        card.id === 'dados_pesquisa' && calculatedOficioHeights.enableScrollPesquisa
                          ? styles.cardWithFixedHeader
                          : ''
                      }`}
                      style={{
                        height:
                          card.id === 'dados_pesquisa' && calculatedOficioHeights.dadosPesquisa
                            ? `${calculatedOficioHeights.dadosPesquisa}px`
                        ...
                        height:
                          ...
                          calculatedOficioHeights.infoComplementares
                            ...
                            : card.id === 'dados_decisao_judicial' &&
                                calculatedOficioHeights.decisaoJudicial
                              ? `${calculatedOficioHeights.decisaoJudicial}px`
                              : card.id === 'dados_midia' && calculatedOficioHeights.dadosMidia
                ...
                {documentosDemanda
                  ...
                  .map(doc => (
                    ...
                    >
                      ...
                      <td className={styles.tableCell}>{doc.destinatario || 'N/A'}</td>
                          ...
                          return (
                            <div
                              style={{
                                width: '12px',
                                height: '12px',
                                backgroundColor: getStatusColor(status as DocumentStatus),
                                borderRadius: '50%',
                                margin: '0 auto',
                              }}
                              title={status}
