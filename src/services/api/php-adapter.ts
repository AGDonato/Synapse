    ...
    (request: Request) => {
      ...
      if (request.method !== 'GET' && request.body) {
        try {
          ...
          logger.warn('Failed to convert request body:', error);
        }
      }
...
export const phpResponseInterceptor = {
  afterResponse: [
    async (request: Request, options: Record<string, unknown>, response: KyResponse) => {
      ...
      try {
        ...
        logger.warn('Failed to convert response:', error);
        return response;
      }
    }
  ],
