    ...
    if (doc.dataResposta) {
      ...
      if (sendDate && responseDate) {
        responseTime = Math.ceil(
          (responseDate.getTime() - sendDate.getTime()) / (1000 * 60 * 60 * 24)
        );
      }
    } else {
...
const OptimizedAverageResponseTimeChart: React.FC<OptimizedAverageResponseTimeChartProps> =
  React.memo(({ filters: externalFilters, height = '400px', loadImmediately = false }) => {
    ...
    const averageData = useOptimizedChartData(
      documentos,
      docs =>
        processAverageResponseTimeData(
          docs,
            ...
            if (paramsArray && paramsArray.length > 0) {
              ...
              return createTooltipHTML({
                ...
                items: [
                  {
                    label: 'Tempo Médio',
                    value: `${provider.averageTime} dias`,
                    color: '#10b981',
                  },
                  {
        ...
        series: [
          {
            ...
            itemStyle: {
              color: (params: EChartsColorParams) => {
                ...
                if (ratio <= 0.3) {
                  return '#52c41a';
                } // Green for fast response
                if (ratio <= 0.6) {
                  return '#faad14';
                } // Yellow for medium response
